.data 
	...
	TOO_LONG_STRING_MESSAGE: .asciiz "String is invalid!"
	ERROR: .asciiz "ERROR!!!\n"
.text
main:
#--------------------------------------------------------------------------------------------
# @brief	Clear string truoc moi lan getString
#--------------------------------------------------------------------------------------------
clearString:
	la $a0,inputString
	loop:	lb $t0,0($a0)
...
isTooLongString:
	...
	syscall
	slti $t2, $s1, 50
	beq $t2, $zero, putErrorStringMessage	# Neu chuoi lon hon 50 thi in error
	blez  $s1,putErrorStringMessage # Neu chuoi nho hon bang 0 thi in error
#--------------------------------------------------------------------------------------------
